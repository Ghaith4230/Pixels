...
public class HexagonTool implements Tool {
    ...
    public void update(Controller ctrl, Point point) {
        if (pressed) {
            ...
            drawHexagon(ctrl, centerPoint, point, ctrl.getColor());
        }
    }
    ...
    private void drawHexagon(Controller ctrl, Point center, Point edge, Color color) {
        // Calculate the radius from the center to the edge point
        double radius = Math.sqrt(Math.pow(center.x() - edge.x(), 2) + Math.pow(center.y() - edge.y(), 2));
    
        for (int i = 0; i < 6; i++) {
            double angle1 = 2 * Math.PI / 6 * i;
            double angle2 = 2 * Math.PI / 6 * (i + 1);
    ...
    //  adjusting a point within canvas boundaries
