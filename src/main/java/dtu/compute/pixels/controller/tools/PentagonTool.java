...
public class PentagonTool implements Tool {
    ...
    public void update(Controller ctrl, Point point) {
        if (pressed) {
            ...
            drawPentagon(ctrl, centerPoint, point, ctrl.getColor());
        }
    }
    ...
    private void drawPentagon(Controller ctrl, Point center, Point edge, Color color) {
        double radius = Math.sqrt(Math.pow(center.x() - edge.x(), 2) + Math.pow(center.y() - edge.y(), 2));
        for (int i = 0; i < 5; i++) {
            double angle1 = 2 * Math.PI / 5 * i;
            double angle2 = 2 * Math.PI / 5 * (i + 1);
            Point p1 = adjustPoint(ctrl, new Point((int) (center.x() + radius * Math.cos(angle1)), 
                                     (int) (center.y() + radius * Math.sin(angle1))));
            Point p2 = adjustPoint(ctrl, new Point((int) (center.x() + radius * Math.cos(angle2)), 
